...
class Calendar(Screen):
    ...
    def get_calendar(self, month, year):
        previous_month = monthcalendar(year - 1 if month == 1 else year, 12 if month == 1 else month)[4]
        next_month = monthcalendar(year + 1 if month == 12 else year, 1 if month == 12 else month)
        calendar = [["pon", "wto", "śrd", "czw", "pią", "sob", "nie"]]

        for i in range(5):
            calendar.append(monthcalendar(2022, 1)[i])
            if i == 0:
                for j in range(7):
                    if calendar[i + 1][j] == 0:
